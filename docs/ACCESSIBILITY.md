...
- **Keyboard operability.** `EntitySwitcher` is built on `@base-ui/react`'s
  `Select` parts (`Select.Root/Trigger/Portal/Positioner/Popup/Item`), not a
  hand-rolled `<button>` + absolutely-positioned `<ul>`. This provides, out
  of the box: a real `<button>` trigger with correct
  `aria-haspopup`/`aria-expanded`, opening on Enter/Space/ArrowDown/ArrowUp
  with the active item focused, roving keyboard navigation (arrow keys,
  Home/End, type-ahead) and selection (Enter/Space) inside the popup,
  Escape and click-outside close, focus restoration to the trigger on
  close, and complete ARIA listbox/option semantics
  (`role="listbox"`/`role="option"`, `aria-selected`) generated by the
  primitive rather than reimplemented by hand. Focus order follows visual
  order in every foundation component.
