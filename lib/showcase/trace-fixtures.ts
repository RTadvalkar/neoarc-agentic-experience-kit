...
// Includes `ConversationEventPayload` (not just Runtime/Trace) so this one
// stream can also produce the two `conversation.message.created` events
// below — the Chat tab's `conversationNodeDefinitions` key off those event
// types specifically, matching the module doc comment's claim that Chat is
// one of the views this scenario renders across (see Gate 5's "Architecture
// Agent Run replays across all four views" in docs/11_V0_GATE_CHECKLIST.md).
export type ArchitectureAgentRunPayload = RuntimeEventPayload | TraceEventPayload | ConversationEventPayload
...
export const architectureAgentRunScenario: TraceExecutionLabScenario = {
  ...
  events: [
    ...
    // The two events below are the Chat tab's raw material: a
    // `conversation.message.created` per author, keyed by `messageId`
    // (never array position — see docs/02A §Replayability). They carry the
    // same `turnId` as the trace/runtime events above so Chat, Activity,
    // Trace, and Provenance all key off the same identities for this turn.
    envelope({
      id: "evt-arch-09b",
      type: "conversation.message.created",
      occurredAt: "2026-08-18T09:00:02.450Z",
      sequence: 10,
      correlation: { ...correlation, turnId: "turn-1" },
      payload: {
        messageId: "msg-arch-user-1",
        author: { id: "user-jamie", kind: "human", displayName: "Jamie Chen" },
        initialContent: [{ kind: "text", text: "Design an async order-events service so downstream teams stop polling the orders table." }],
      },
    }),
    envelope({
      id: "evt-arch-09c",
      type: "conversation.message.completed",
      occurredAt: "2026-08-18T09:00:02.500Z",
      sequence: 11,
      correlation: { ...correlation, turnId: "turn-1" },
      payload: { messageId: "msg-arch-user-1", status: "completed" },
    }),
    envelope({
      id: "evt-arch-10",
    ...
    // Agent's closing reply for the turn: the Chat tab's second (and last)
    // message, summarizing the ADR decision the Trace/Provenance/Mission
    // tabs already recorded in full detail above.
    envelope({
      id: "evt-arch-35b",
      type: "conversation.message.created",
      occurredAt: "2026-08-18T09:00:40.750Z",
      sequence: 38,
      correlation: { ...correlation, turnId: "turn-1" },
      payload: {
        messageId: "msg-arch-agent-1",
        author: { id: "agent-architecture", kind: "agent", displayName: "Architecture agent" },
        initialContent: [
          {
            kind: "text",
            text: "Designed the order-events service on SQS fan-out (ADR-0042, approved) and provisioned the sandbox queue. Boundary and diagram artifacts are attached.",
          },
        ],
      },
    }),
    envelope({
      ...
      sequence: 39,
      correlation: { ...correlation, turnId: "turn-1" },
      payload: { messageId: "msg-arch-agent-1", status: "completed", artifacts: [{ id: "artifact-adr-42", name: "ADR-0042: Adopt SQS-based order-events service", artifactType: "document" }, { id: "artifact-diagram-42", name: "order-events-service architecture diagram", artifactType: "diagram" }] },
    }),
    envelope({
      id: "evt-arch-36",
      type: "step.completed",
      occurredAt: "2026-08-18T09:00:40.900Z",
      sequence: 40,
      correlation: { ...correlation, turnId: "turn-1", stepId: "step-2" },
      payload: {},
    }),
